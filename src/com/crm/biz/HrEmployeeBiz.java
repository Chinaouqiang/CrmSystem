...
public class HrEmployeeBiz {
	...
	public boolean deleteFlase(int id) {
		...
		employee.setDeleteTime(new Timestamp(new Date().getTime()));
		try {
	...
	public Boolean Addlogon(HrEmployee hrEmployee) {
		try {
	...
	//根据序列号查询店铺对应的员工
		...
		public List<Map<String, Object>> findList(String Only){
			List<Map<String, Object>> ss=hrEmployeeDao.findList(Only);
			System.out.println("根据序列号查询店铺对应的员工:"+ss);
			return ss ;
		}
		
	//根据序列号查询店铺对应的员工人数
	public Object findCount(String Only){
		Object ss=hrEmployeeDao.findCount(Only);
		System.out.println("根据序列号查询店铺对应的员工人数:"+ss);
		return ss;
	}
		
	
	//
	public List<Map<String, Object>> findCheck(String Name,String Path) {
		List<Map<String, Object>> s=hrEmployeeDao.findList2(Name, Path);
		return s;
	}
}
